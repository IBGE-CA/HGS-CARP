...
void Genetic::evolveHGA(int maxIterNonProd, int nbRec)
{
	...
	SolutionCost bestSolFeasibility;
	...
	if (population->getBestFeasibleIndividual() != NULL) bestSolFeasibility = population->getBestFeasibleIndividual()->solutionCost;
	else bestSolFeasibility = population->getBestInfeasibleIndividual()->solutionCost;
	for (int i = 0; i < population->unfeasible.nbIndiv; i++)
		if (population->unfeasible.individuals[i]->solutionCost.isBetterFeas(bestSolFeasibility)) bestSolFeasibility = population->unfeasible.individuals[i]->solutionCost;
	...
	while (nbIterNonProd < maxIterNonProd && (clock() - debut <= ticks) && (!params->isSearchingFeasible || population->getBestFeasibleIndividual() == NULL))
	{
		// CROSSOVER
		parent1 = population->getIndividualByBinTourn(); // Pick two individuals per binary tournament
		parent2 = population->getIndividualByBinTourn(); // Pick two individuals per binary tournament
		...
		rejeton->updateIndiv();
		population->updateValidityCounters(rejeton);
		place = population->addIndividual(rejeton);

		// POSSIBLE REPAIR
		if (!rejeton->isValide)
		{
			fitBeforeRepair = rejeton->solutionCost.evaluation;
			if (rand() % 2 == 0) // 50% chance to do repair on an infeasible individual
			{
				reparer();
				if (rejeton->solutionCost.evaluation < fitBeforeRepair - 0.01 || rejeton->solutionCost.evaluation > fitBeforeRepair + 0.01 || rejeton->isValide)
					place2 = population->addIndividual(rejeton);
				if (rejeton->isValide)
					place = place2;
				else
		...
		// SOME TRACES
		if ((rejeton->isValide && place == 0) || (rejeton->solutionCost.isBetterFeas(bestSolFeasibility) && population->feasible.nbIndiv == 0))
		{
			if (traces && population->feasible.nbIndiv > 0)
				cout << "NEW BEST FEASIBLE " << place << " " << population->getBestFeasibleIndividual()->solutionCost.evaluation << " distance : " << rejeton->solutionCost.distance << " nbRoutes : " << rejeton->solutionCost.routes << " capaViol : " << rejeton->solutionCost.capacityViol << " lengthViol : " << rejeton->solutionCost.lengthViol << endl << endl;
			if (traces && population->feasible.nbIndiv == 0)
				cout << "NEW BEST INFEASIBLE " << place << " " << rejeton->solutionCost.evaluation << " distance : " << rejeton->solutionCost.distance << " nbRoutes : " << rejeton->solutionCost.routes << " capaViol : " << rejeton->solutionCost.capacityViol << " lengthViol : " << rejeton->solutionCost.lengthViol << endl << endl;
			if (rejeton->solutionCost.isBetterFeas(bestSolFeasibility))
				bestSolFeasibility = rejeton->solutionCost;
			nbIterNonProd = 1;
		}
		else nbIterNonProd++;
...
void Genetic::evolveILS()
{
	...
	nbIter = 0;
	clock_t debut = clock();
	rejetonBestFoundAll->solutionCost.evaluation = 1.e30;
	...
	for (int phaseGrasp = 0; phaseGrasp < nbGRASP; phaseGrasp++)
	{
		...
		delete myIndiv;
		rejetonBestFound->solutionCost.evaluation = 1.e30;
		isFirstLoop = true;
		...
		for (int nbGenerationNonProd = 0; nbGenerationNonProd < nbILS && (clock() - debut <= ticks); nbGenerationNonProd++)
		{
			if (!isFirstLoop)
			{
				parent = population->getBestFeasibleIndividual();
				if (parent == NULL) parent = population->getBestInfeasibleIndividual();
			...
			for (int i = 0; i < nbCHILD; i++)
			{
				...
				if (!rejeton->isValide) reparer();
				population->addIndividual(rejeton);

				// Checking if its a solution improvement
				if (rejeton->isValide && rejeton->solutionCost.evaluation < rejetonBestFound->solutionCost.evaluation - 0.001)
				{
					nbGenerationNonProd = -1;
					rejetonBestFound->recopieIndividu(rejetonBestFound, rejeton);
					if (rejetonBestFound->solutionCost.evaluation < rejetonBestFoundAll->solutionCost.evaluation - 0.001)
					{
						cout << "NEW BEST EVER : " << rejetonBestFound->solutionCost.evaluation << endl;
						rejetonBestFoundAll->recopieIndividu(rejetonBestFoundAll, rejetonBestFound);
						timeBest2 = population->timeBest;
					}
					else
						cout << "NEW BEST      : " << rejeton->solutionCost.evaluation << endl;
				}
	...
	population->addIndividual(rejetonBestFoundAll);
...
void Genetic::gererPenalites()
{
	...
	double fractionCharge = population->fractionOfLoadFeasibleIndividuals();
	double fractionTemps = population->fractionOfTimeFeasibleIndividuals();
	...
	if (changeDone)
		population->validatePen(population->unfeasible);
}
...
void Genetic::crossPIX()
{
	...
	for (i = 0; i < params->nbClients + cli.nbDepots; i++)
	{
		freqClient[i] = params->clients[i].freq;
		rejeton->chromP[i].pat = 0;
		rejeton->chromP[i].dep = -1;
	}

	int nbDays = cli.getNbDays();
	...
	for (int k = 0; k < nbDays; k++)
	{
		...
		if (k < j1 && !rejeton->chromT[day].empty())
		{
			...
			while (j != (int)((fin + 1) % rejeton->chromT[day].size()))
			{
				...
				rejeton->chromP[rejeton->chromT[day][j]].pat += (int)pow((float)2, (int)((nbDays - day) % params->oldNbDays));
				rejeton->chromP[rejeton->chromT[day][j]].dep = (day - 1) / params->oldNbDays;
	...
	for (int k = 0; k < nbDays; k++)
	{
		...
		if (k < j2)
		{
			for (i = 0; i < (int)rejeton2->chromT[day].size(); i++)
			{
				...
				if (freqClient[ii] != 0
					&& params->clients[ii].jourSuiv[rejeton->chromP[ii].pat][(day - 1) % params->oldNbDays + 1] == (int)((day - 1) % params->oldNbDays + 1)
					&& (rejeton->chromP[ii].dep == -1 || rejeton->chromP[ii].dep == (day - 1) / params->oldNbDays))
				{
					rejeton->chromT[day].push_back(ii);
					freqClient[ii] -= 1;
					rejeton->chromP[ii].pat += (int)pow((float)2, (int)((nbDays - day) % params->oldNbDays));
					rejeton->chromP[ii].dep = (day - 1) / params->oldNbDays;
				}
			}
		}
	}
	...
	ticks(ticks), traces(traces), population(population), params(params)
{
	for (int i = 0; i < params->nbClients + cli.nbDepots; i++)
		freqClient.push_back(params->clients[i].freq);

	// Creating the Individuals that serve to perform the Local Search and other operations
	rejeton = new Individual(params, true);
	rejeton2 = new Individual(params, true);
	rejetonP1 = new Individual(params, true);
	rejetonP2 = new Individual(params, true);
	rejetonBestFound = new Individual(params, true);
	rejetonBestFoundAll = new Individual(params, true);
