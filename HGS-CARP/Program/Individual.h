...
struct SolutionCost
{
	...
	bool isBetterFeas(SolutionCost c2) const
	{
		bool isBetter = false;
...
public:
	...
	std::vector<std::vector<int>> chromT;

	// Pattern chromosome of an individual
	std::vector<Pattern> chromP;
	...
	// successors[clients][day] gives the next customer in the considered day
	// Used to compute the Hamming distance between solutions
	// if the customer does not exist in this day, returns -1
	std::vector<std::vector<int>> successors;

	// predecessors[clients][day] gives the previous customer in the considered day
	// Used to compute the Hamming distance between solutions
	// if the customer does not exist in this day, returns -1
	std::vector<std::vector<int>> predecessors;

	// computing the successors and predecessors tables
	void computeSuccessors();

	// working table for split (dynamic programming for Split)
	std::vector<std::vector<SolutionCost>> potentials;
	...
	std::vector<std::vector<SolutionCost>> splitArcsCosts;
	...
	// std::vector used in crossover PIX
	...
	// initialization of the potentials std::vector for Split
