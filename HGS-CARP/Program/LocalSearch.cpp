...
void LocalSearch::runSearchTotal()
{
	...
	if (params->periodique || cli.type == Data::MDCARP)
	{
...
int LocalSearch::mutationSameDay(int day)
{
	...
	while (!rechercheTerminee)
	{
		...
		for (int posU = 0; posU < (int)ordreParcours[day].size(); posU++)
		{
			...
			costAfterRemoval = noeudU->seq0_i->evaluation(noeudU->pred->seq0_i, noeudU->next->seqi_n, routeU->vehicle);
			...
			if (!noeudV->route->nodeAndRouteTested[noeudU->cour])
			{
				while (moveEffectue != 1 && !noeudV->isDepot)
				{
					// Testing 2-Opt between U and V (if the restriction of the granular search allows) 
					if (params->isCorrelated[noeudU->pred->cour][noeudV->cour] || params->isCorrelated[noeudU->cour][noeudV->next->cour])
			...
			for (int route = 0; route < params->nbVehiculesPerDep && moveEffectue == 0; route++)
			{
				...
				routeV = noeudV->route;
				y = noeudV->next;
				if ((!noeudV->route->nodeAndRouteTested[noeudU->cour]) && (!y->isDepot || !routeVideTestee))
				{
					if (y->isDepot) routeVideTestee = true;
					if (routeV != routeU)
					{
						tempNoeud = noeudV;
						noeudV = depots[day][route].next;
						y = noeudV->next;
...
int LocalSearch::interRouteGeneralInsert()
{
	...
	if (!noeudU->isDepot)
	{
		...
		if (!x->isDepot)
		{
			resultMoves[2][0] = seq->evaluationLB(noeudU->pred->seq0_i, x->next->seqi_n, routeU->vehicle);
			resultMoves[3][0] = resultMoves[2][0];
			if (!noeudV->isDepot)
			{
				resultMoves[2][1] = seq->evaluationLB(noeudU->pred->seq0_i, noeudV->seq1, x->next->seqi_n, routeU->vehicle);
				resultMoves[3][1] = resultMoves[2][1];
				if (!y->isDepot)
				{
					resultMoves[2][2] = seq->evaluationLB(noeudU->pred->seq0_i, noeudV->seq12, x->next->seqi_n, routeU->vehicle);
					resultMoves[2][3] = seq->evaluationLB(noeudU->pred->seq0_i, noeudV->seq21, x->next->seqi_n, routeU->vehicle);
	...
	if (!noeudV->isDepot)
	{
		...
		if (!y->isDepot && !noeudU->isDepot && !x->isDepot)
		{
			temp = seq->evaluationLB(noeudV->pred->seq0_i, noeudU->seq12, y->next->seqi_n, routeV->vehicle);
			resultMoves[2][2] += temp;
			resultMoves[2][3] += temp;
			temp = seq->evaluationLB(noeudV->pred->seq0_i, noeudU->seq21, y->next->seqi_n, routeV->vehicle);
	...
	if (!noeudU->isDepot)
	{
		...
		if (!noeudV->isDepot && shouldBeTested[1][1])
		...
		if (!x->isDepot)
		{
			...
			{
				resultMoves[2][0] = seq->evaluation(noeudU->pred->seq0_i, x->next->seqi_n, routeU->vehicle);
				resultMoves[3][0] = resultMoves[2][0];
			}
			if (!noeudV->isDepot)
			{
				...
				{
					resultMoves[2][1] = seq->evaluation(noeudU->pred->seq0_i, noeudV->seq1, x->next->seqi_n, routeU->vehicle);
					resultMoves[3][1] = resultMoves[2][1];
				}
				if (!y->isDepot)
				{
					...
					{
						resultMoves[2][2] = seq->evaluation(noeudU->pred->seq0_i, noeudV->seq12, x->next->seqi_n, routeU->vehicle);
						resultMoves[3][2] = resultMoves[2][2];
					}
					if (shouldBeTested[2][3] || shouldBeTested[3][3])
					{
						resultMoves[2][3] = seq->evaluation(noeudU->pred->seq0_i, noeudV->seq21, x->next->seqi_n, routeU->vehicle);
						resultMoves[3][3] = resultMoves[2][3];
					}
				}
			}
	...
	if (!noeudV->isDepot)
	{
		...
		if (!y->isDepot && !noeudU->isDepot && !x->isDepot)
		{
			...
			{
				temp = seq->evaluation(noeudV->pred->seq0_i, noeudU->seq12, y->next->seqi_n, routeV->vehicle);
				resultMoves[2][2] += temp;
				resultMoves[2][3] += temp;
			}
			if (shouldBeTested[3][2] || shouldBeTested[3][3])
			{
				temp = seq->evaluation(noeudV->pred->seq0_i, noeudU->seq21, y->next->seqi_n, routeV->vehicle);
				resultMoves[3][2] += temp;
				resultMoves[3][3] += temp;
			}
		}
	...
	Node* placeV = noeudV->pred;
	Node* placeU = noeudU->pred;
	...
	setRouteVide(noeudU->day); // Keep a pointer on the first empty route
...
int LocalSearch::interRoute2Opt()
{
	...
	if (routeU->depot->cour != routeV->depot->cour || (noeudU->pred->isDepot && noeudV->isDepot))
...
int LocalSearch::interRoute2OptInv()
{
	...
	if (y->isDepot)
	{
		depotVFin->next = depotV;
		depotVFin->pred = depotUSuiv;
		depotVFin->pred->next = depotVFin;
		depotU->next = x;
		x->pred = depotU;
	}
	else if (noeudU->isDepot)
	{
		depotU->next = depotVFin->pred;
		depotU->next->pred = depotU;
...
int LocalSearch::intraRouteGeneralInsertDroite()
{
	...
	{
		if (!noeudV->isDepot && turned) // relocate of V before U (V cannot be a depot, but U can be)
		{
			myseqs.clear();
			...
			addSeqDataInPieces(noeudU, noeudV->place - 1 - noeudU->place, noeudU->day);
		...
		if (!x->isDepot)
		{
			if (!turned)
			{
				myseqs.clear();
				myseqs.push_back(Upred->seq0_i);
				addSeqDataInPieces(xsuiv, noeudV->place - 1 - xsuiv->place, noeudU->day);
				myseqs.push_back(noeudU->seq12);
				myseqs.push_back(noeudV->seqi_n);
				resultMoves[2][0] = seq->evaluationLB(myseqs, routeU->vehicle);
			}

			if (!noeudV->isDepot) // exchange U,X and V
			{
				if (!turned)
				{
					myseqs.clear();
			...
			if (!noeudV->isDepot) // exchange X,U and V
			{
				if (!turned)
				{
					myseqs.clear();
...
int LocalSearch::intraRoute2Opt()
{
	...
	Node* nodeNum = noeudU->next;
	Node* nodeUpred = noeudU->pred;
	Node* temp;
	SeqData* seq = noeudU->seq0_i;
	...
	addReverseSeqDataInPieces(noeudU, noeudV->place - noeudU->place, noeudU->day);
	myseqs.push_back(noeudV->next->seqi_n);
	...
	{
		temp = nodeNum->next;
		nodeNum->next = nodeNum->pred;
		nodeNum->pred = temp;
		nodeNum = temp;
	}

	noeudV->next = noeudV->pred;
	noeudV->pred = nodeUpred;
	nodeUpred->next = noeudV;
	y->pred = noeudU;
	routeU->updateRouteData(false);  // Update the pre-processed data on the subsequences of the route
	setRouteVide(noeudU->day); // Keep a pointer on the first empty route
	rechercheTerminee = false; // Not finished the search
...
int LocalSearch::searchBetterPattern(int client)
{
	...
	for (int pat = 0; pat < (int)params->clients[client].visits.size(); pat++)
	{
		// testing a new Pattern
		pattern2 = params->clients[client].visits[pat];
		...
		for (int k = 0; k < params->oldNbDays; k++)
		{
			...
			if (temp == 1)
			{
				noeudTravail = &clients[params->oldNbDays - k + depot * params->oldNbDays][client];
	...
	if (meilleurPattern.pat != pattern1.pat || meilleurPattern.dep != pattern1.dep || deplacementIntraJour)
	{
		...
		for (int k = 0; k < params->oldNbDays; k++)
		{
			...
			calcul = calcul / 2;
			if (temp == 1)
				removeNoeud(&clients[params->oldNbDays - k + pattern1.dep * params->oldNbDays][client]);
		}
		...
		for (int k = 0; k < params->oldNbDays; k++)
		{
			...
			if (temp == 1)
			{
				Node* hereCli = &clients[params->oldNbDays - k + meilleurPattern.dep * params->oldNbDays][client];
				Node* thereCli = hereCli->placeInsertion[indexMeilleur];
				addNoeud(hereCli, thereCli);
			}
		}
		//cout << "Inserting Node " << clients << " With Pattern " << meilleurPattern.pat << " Flag is : " << deplacementIntraJour << endl ;
...
void LocalSearch::computeCoutInsertion(Node* clients, int Pattern)
{
	// Computing the best cost for inserting a clients in its day
	Route* myRoute;
	clients->coutInsertion[Pattern] = 1.e30;
	clients->placeInsertion[Pattern] = NULL;
	...
	for (int r = 0; r < params->nombreVehicules[clients->day]; r++)
	{
		myRoute = &routes[clients->day][r];
		if (myRoute->coutInsertionClient[clients->cour][Pattern] > 1.e29 || firstLoop)
			evalInsertClient(myRoute, clients, Pattern);

		if (myRoute->coutInsertionClient[clients->cour][Pattern] < clients->coutInsertion[Pattern] - EPSILON_LS)
		{
			clients->coutInsertion[Pattern] = myRoute->coutInsertionClient[clients->cour][Pattern];
			clients->placeInsertion[Pattern] = myRoute->placeInsertionClient[clients->cour][Pattern];
		}
	}
	...
	if (clients->isPresent // its placed here
		&& testingIncumbentPattern // with the same Pattern
		&& clients->route->coutInsertionClient[clients->cour][Pattern] > clients->coutInsertion[Pattern] + EPSILON_LS) // but we can do better
		deplacementIntraJour = true;
}

void LocalSearch::evalInsertClient(Route* R, Node* U, int Pattern)
{
	// Computing the least cost for inserting a clients U in a route R
	...
	U->seq1->load = params->clients[U->cour].demandPatDay[params->clients[U->cour].visits[Pattern].pat][U->day];
	...
	else
	{
		...
		leastCost = seq->evaluation(U->pred->seq0_i, U->seq1, U->next->seqi_n, R->vehicle);
		...
		while (!courNoeud->isDepot || firstLoopDep)
		{
			...
			if (courNoeud->place < U->place)
			{
				...
				myseqs.push_back(U->next->seqi_n);
			...
			else if (courNoeud->place > U->place + 1)
			{
				...
				addSeqDataInPieces(U->next, courNoeud->place - 1 - U->next->place, courNoeud->day);
				myseqs.push_back(U->seq1);
		...
		R->coutInsertionClient[U->cour][Pattern] = leastCost - seq->evaluation(U->pred->seq0_i, U->next->seqi_n, R->vehicle);
	}
...
void LocalSearch::updateMoves()
{
	...
	for (int k = 1; k <= cli.getNbDays(); k++)
	{
		for (int i = 0; i < (int)ordreParcours[k].size(); i++)
		{
			...
			size = (int)params->clients[client].sommetsVoisinsAvant.size();
			for (int a1 = 0; a1 < size; a1++)
			{
				client2 = params->clients[client].sommetsVoisinsAvant[a1];
				if (client2 >= cli.nbDepots && clients[k][client2].isPresent) clients[k][client].moves.push_back(client2);
			}
		}
	}
}
...
void LocalSearch::setRouteVide(int day)
{
	...
	{
		if (depots[day][route].next->isDepot) routeVide[day] = depots[day][route].route;
		route++;
	}
}
...
void LocalSearch::placeManquants()
{
	...
	for (int i = 0; i < (int)individu->toPlace.size(); i++)
	{
		...
		for (int pat = 0; pat < (int)params->clients[k].visits.size(); pat++)
		{
			// testing a new Pattern
			pattern2 = params->clients[k].visits[pat];
			if (Pattern::isSubset(pattern1, pattern2))
			{
				calcul2 = pattern2.pat;
				depense = pattern2.cost;
				for (int kk = 0; kk < params->oldNbDays; kk++)
				{
					if (calcul2 % 2 == 1)
					{
						noeudTravail = &clients[params->oldNbDays - kk + pattern2.dep * params->oldNbDays][k];
						computeCoutInsertion(noeudTravail, pat);
						depense += noeudTravail->coutInsertion[pat];
					}
					calcul2 = calcul2 / 2;
		...
		for (int kk = 0; kk < params->oldNbDays; kk++)
		{
			...
			{
				Node* hereCli = &clients[params->oldNbDays - kk + meilleurPattern.dep * params->oldNbDays][k];
				Node* thereCli = hereCli->placeInsertion[indexMeilleur];
				addNoeud(hereCli, thereCli);
			}
			calcul1 = calcul1 / 2;
...
void LocalSearch::addOP(int day, int clients)
{
	...
	if (ordreParcours[day].size() != 0)
	{
		...
		ordreParcours[day][it] = clients;
		ordreParcours[day].push_back(temp2);
	}
	else
		ordreParcours[day].push_back(clients);
}
...
void LocalSearch::reinitSingleDayMoves(Route* r)
{
	...
	for (Node* tempNoeud = r->depot->next; !tempNoeud->isDepot; tempNoeud = tempNoeud->next)
...
bool LocalSearch::ejectionChains(int day)
{
			...
			for (int iiPrev = 1; iiPrev < ec_nbElements[rPrev]; iiPrev++)
			{
				...
				for (int ii = 0; ii < ec_nbElements[r]; ii++)
				{
					myDeltaCost = seq->evaluation(ejectionGraph[r][ii].myNode->seq0_i,
						...
						ejectionGraph[r][ii].myNode->next->seqi_n,
	...
	if (orderEnds[0]->cost < -0.0001)
	{
		...
		for (int myID = 0; myID < 1; myID++)
		{
			...
			Node* insertionPosition = NULL;
			Node* insertionPositionTemp = NULL;
...
void LocalSearch::addSeqDataInPieces(Node* node, int length, int day)
{
	...
	while (courLenght > 0)
	{
		...
		courLenght -= temp;
		if (courLenght != 0) cour = clients[day][courSeq->lastNode].next;
	}
}
...
void LocalSearch::addReverseSeqDataInPieces(Node* node, int length, int day)
{
	...
	while (courLenght > 0)
	{
		...
		courLenght -= temp;
		if (courLenght != 0) cour = clients[day][courSeq->firstNode].next;
	}
...
LocalSearch::LocalSearch(Params* params, Individual* individu) : params(params), individu(individu)
{
	...
	clients = new Node*[cli.getNbDays() + 1];
	depots = new Node*[cli.getNbDays() + 1];
	depotsFin = new Node*[cli.getNbDays() + 1];
	routes = new Route*[cli.getNbDays() + 1];

	for (int kk = 1; kk <= cli.getNbDays(); kk++)
	{
		nbVehicles = params->nombreVehicules[kk];
		clients[kk] = new Node[params->nbClients + cli.nbDepots + 1];
		depots[kk] = new Node[nbVehicles];
		depotsFin[kk] = new Node[nbVehicles];
		routes[kk] = new Route[nbVehicles];
	...
	for (int day = 0; day <= cli.getNbDays(); day++)
	{
	...
	int nbSeqsSet = 0;
	int taillemyseqDatas = (params->sizeSD + params->sizeSD + 4) * (params->nbClients + cli.nbDepots + 2 * params->nbVehiculesPerDep) * cli.getNbDays();
