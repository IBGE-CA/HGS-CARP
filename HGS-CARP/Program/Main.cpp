...
int main(int argc, char* argv[])
{
	try
	{
		...
		if (!cli.isMinFleetSize() && !cli.isMinMaxTour())
		{
			clock_t nbTicksAllowed = cli.timeLimit * CLOCKS_PER_SEC;
			...
			Params myParams(false);

			// Running the algorithm
			Population population(myParams);
			Genetic solver(&myParams, &population, nbTicksAllowed, true);
		...
		//else
		//{
		//	int veh = cli.nbVehicles; // start with an upper bound on the number of vehicles
		...
		//		mesParametresTab.push_back(Params(true));
		...
		//		if (cli.isMinFleetSize() && cli.isMinMaxTour()) throw string("This program was not designed to optimize jointly the fleet size and length of the maximum tour");
		...
		//		if (nbpop >= 2) populationTab[nbpop - 1].addAllIndividuals(&populationTab[nbpop - 2]);
		//		if (nbpop >= 3) populationTab[nbpop - 1].addAllIndividuals(&populationTab[nbpop - 3]);
		//		// Solving
		...
		//		if (populationTab[nbpop - 1].getBestFeasibleIndividual() != NULL)
		//		{
		//			if (cli.isMinFleetSize())
		//				veh--; // reducing the fleet size (PCARP)
		//			else if (cli.isMinMaxTour())
		//				distConstraint = populationTab[nbpop - 1].getBestFeasibleIndividual()->maxRoute - 1; // or reducing the distance below the best current solution (MM-kWRPP)
		//		}
		//		else
		//			validExist = false;
		...
		//		if (cli.type == Data::PCARP && mesParametresTab[nbpop - 1].totalDemand > veh *
		...
		//		Params mesParametres(cli, false);
		//		Population population(mesParametres);
		//		if (nbpop >= 1 && populationTab[nbpop - 1].getBestFeasibleIndividual() != NULL)
		//			population.addAllIndividuals(&populationTab[nbpop - 1]);
		//		else if (nbpop >= 2)
		//			population.addAllIndividuals(&populationTab[nbpop - 2]);
