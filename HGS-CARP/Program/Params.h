...
public:
	...
	type =     // This lists the problems which can be solved with this algorithm
			   ...
			   31 NEARP (Edges, Arcs and Nodes), also called NEARP in the literature
	...
	*/
	...
	bool hasTurnPenalties ; // is there turn penalties
	//bool hasTimeLimit ; // is the problem time-capacitated (problem in which all costs and the vehicle capacity are time-based)
	...
	std::vector<std::vector<double>> ar_distanceNodes;

	// predecessors in the shortest paths between nodes of the original network
	std::vector<std::vector<int>> ar_predNodes;
	...
	int ar_nbTurns;
	int ar_maxNbModes;
	std::vector<Arc> ar_Arcs;
	std::vector<std::vector<Arc*>> ar_correspondingArc; // correspondingArc[i][j] returns a pointer to the associated arc between node i and j, otherwise, if non existing an error.
	std::vector<std::vector<double>> ar_distanceArcs;
	...
	// number of days
	int oldNbDays; // copy of the number of days

	// number of vehicles per depot
	int nbVehiculesPerDep;

	// for each day, sum of the capacity of the vehicles
	std::vector<double> dayCapacity;

	// list of vehicles available for each day
	std::vector<std::vector<Vehicle>> ordreVehicules;

	// number of vehicles available for each day
	std::vector<int> nombreVehicules;

	// array containing the information of each separate clients/service
	Client* clients;
	...
	std::vector<std::vector<bool>> isCorrelated;
	...
	// constructor
	Params(bool isSearchingFeasible);
	//Params(const std::string& nomInstance, const std::string& nomSolution, const std::string& nomBKS, int seedRNG,
	//	int type, bool hasTimeLimit, bool soft, int nbVehicles, int nbDepots, bool isSearchingFeasible, bool deadheadingArcs);
