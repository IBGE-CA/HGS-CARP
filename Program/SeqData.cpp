...
void SeqData::initialisation(int Ucour, Params* mesParams, Individual* myIndiv, int day, bool isForPathTracking)
{
	...
	for (int i = 0; i < params->clients[Ucour].ar_nbModes; i++)
	{
		for (int j = 0; j < params->clients[Ucour].ar_nbModes; j++)
		{
			if (i == j)
				bestCost[i][j] = 0.0;
			else
	...
	if (isForPathTracking)
	{
		for (int i = 0; i < params->clients[Ucour].ar_nbModes; i++)
		{
			for (int j = 0; j < params->clients[Ucour].ar_nbModes; j++)
			{
				...
				if (i == j)
					bestCostArcs[i][j].push_back(pair<int, int>(params->clients[Ucour].ar_Modes[i]->nodeBegin, params->clients[Ucour].ar_Modes[i]->nodeEnd));
			}
		}
	}
...
void SeqData::concatOneAfterWithPathTracking(SeqData* seq, int Vcour, Individual* myIndiv, int day)
{
	...
	for (int i = 0; i < firstCli->ar_nbModes; i++)
	{
		for (int j = 0; j < vCourCli->ar_nbModes; j++)
		{
			...
			bestCostArcs[i][j].push_back(pair<int, int>(params->clients[Vcour].ar_Modes[j]->nodeBegin, params->clients[Vcour].ar_Modes[j]->nodeEnd));
		}
	}
...
double SeqData::evaluation(SeqData* seq1, SeqData* seq2, SeqData* seq3, Vehicle* vehicle)
{
	Client* cliLast1 = &params->clients[seq1->lastNode];
	Client* cliLast2 = &params->clients[seq2->lastNode];
	Client* cliFirst2 = &params->clients[seq2->firstNode];
	Client* cliFirst3 = &params->clients[seq3->firstNode];
...
double SeqData::evaluation(vector<SeqData*> seqs, Vehicle* vehicle)
{
	...
	for (int s = 1; s < (int)seqs.size(); s++)
	{
		seqbPred = seqs[s - 1];
		seqb = seqs[s];
		cliPredLast = &params->clients[seqbPred->lastNode];
		cliFirst = &params->clients[seqb->firstNode];
		cliLast = &params->clients[seqb->lastNode];
		loadTemp += seqb->load;
