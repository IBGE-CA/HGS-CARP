...
public:
	...
	// suivants[clients][day] gives the next customer in the considered day
	// Used to compute the Hamming distance between solutions
	// if the customer does not exist in this day, returns -1
	std::vector<std::vector<int>> suivants;

	// suivants[clients][day] gives the previous customer in the considered day
	// Used to compute the Hamming distance between solutions
	// if the customer does not exist in this day, returns -1
	std::vector<std::vector<int>> precedents;

	// computing the suivants and precedents tables
	void computeSuivants();

	// working table for split (dynamic programming for Split)
	std::vector<std::vector<CoutSol>> potentiels;
	...
	std::vector<std::vector<CoutSol>> coutArcsSplit;
