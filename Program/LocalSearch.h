...
private:
	...
	std::vector<SeqData*> myseqs; // temporary vector to keep some pointers towards preprocessed data
	...
	std::vector<std::vector<double>> resultMoves;

	// used during moves preprocessing, using the lower bounds
	std::vector<std::vector<bool>> shouldBeTested;
	...
	// change the Pattern of "clients" if its possible to find a better Pattern (PI)
	int searchBetterPattern(int clients);
	void computeCoutInsertion(Noeud* clients, int Pattern); // subprocedures for PI
...
public:
	...
	std::vector<std::vector<int>> ordreParcours;
	void addOP(int day, int clients); // functions for updating the ordreParcours data structure
	void removeOP(int day, int clients); // functions for updating the ordreParcours data structure
	...
	int ec_nbRoutes;
	std::vector<int> ec_nbElements;
	std::vector<std::vector<EC_element>> ejectionGraph;
	...
	void insertNoeud(Noeud* U, Noeud* V); // Inserts clients U after V
	void removeNoeud(Noeud* U); // Removes clients U
	void addNoeud(Noeud* U, Noeud* V); // Adds clients U after V
