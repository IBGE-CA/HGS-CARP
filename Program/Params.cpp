...
Params::Params(bool isSearchingFeasible) : cli(CommandLineInterface::getInstance()), data(Data::getInstance())
{
	...
	//// one for each depot
	//if (cli.type == Data::MDCARP) processDataStructuresMD();
...
Params::~Params(void)
{
	for (int i = 0; i < nbClients + cli.nbDepots; i++)
		delete[] timeCost[i];
	delete[] timeCost;
	delete[] clients;
}
...
void Params::loadData(const CommandLineInterface& cli)
{
	...
	totalDemand = 0;
	periodique = false;
	hasTurnPenalties = false;
	char* myChars = new char[100];

	// This is a standard CARP (can also be an experiment for MDCARP, when the number of depots is defined to be greater than 1)
	if (cli.type == Data::CARP || cli.type == Data::MDCARP)
	{
	...
	// This is the standard NEARP (also called NEARP)
	else if (cli.type == Data::NEARP)
	{
		getline(fichier, contenu);
		getline(fichier, contenu);
		int nodesTotal;
	...
	// This is the PCARP (file format provided by N. Labadi)
	else if (cli.type == Data::PCARP)
	{
		periodique = true;
		// Reading all lines one by one
	...
	// This is the NEARP-TP (also called NEARP-TP)
	else if (cli.type == Data::NEARP_TP)
	{
		hasTurnPenalties = true;
	...
	// Min-Max WRPP, instance format of Angel Corberan
	else if (cli.type == Data::MM_kWRPP)
	{
		getline(fichier, contenu);
		getline(fichier, contenu);
		fichier >> useless2;
	...
	for (int kk = 1; kk <= cli.getNbDays(); kk++)
	{
		ordreVehicules.push_back(tempI);
		dayCapacity.push_back(0);
		nombreVehicules.push_back(cli.nbDepots * nbVehiculesPerDep);
		for (int i = 0; i < cli.nbDepots; i++)
		{
			for (int j = 0; j < nbVehiculesPerDep; j++)
			{
				if (cli.timeCapacitated)
	...
	clients = new Client[cli.nbDepots + nbClients];
	for (int i = 0; i < nbClients + cli.nbDepots; i++)
		getClient(i, &clients[i]);

	// The file formats for CARP, NEARP and NEARP-TP may require some distinct parsing routines due to different formats
	if (cli.type == Data::CARP || cli.type == Data::MDCARP || cli.type == Data::MM_kWRPP) // CARP or MM-kWRPP
	{
		ar_parseOtherLinesCARP();
		ar_computeDistancesNodes(); // We only need the distance between endpoints
	}
	else if (cli.type == Data::NEARP) // NEARP
	{
		ar_parseOtherLinesNEARP();
		ar_computeDistancesNodes();  // We only need the distance between endpoints
	}
	else if (cli.type == Data::PCARP) // PCARP
	{
		ar_computeDistancesNodes(); // We only need the distance between endpoints
	}
	else if (cli.type == Data::NEARP_TP) // NEARP-TP
	{
...
void Params::calculeStructures()
{
	...
	for (int i = 0; i < nbClients + cli.nbDepots; i++)
	{
		...
		for (int j = 0; j < nbClients + cli.nbDepots; j++)
			isCorrelated[i].push_back(i < cli.nbDepots || j < cli.nbDepots);
	}

	for (int i = 0; i < nbClients + cli.nbDepots; i++)
	{
		clients[i].sommetsVoisins.clear();
		clients[i].sommetsVoisinsAvant.clear();
	}
	...
	for (int i = 0; i < nbClients + cli.nbDepots; i++)
	{
		...
		sort(myVector.begin(), myVector.end(), compPredicate);

		// And keeping only the closest ones
		for (int j = 0; j < min(nbClients, granularity); j++)
		{
			clients[i].sommetsVoisinsAvant.push_back(myVector[j].myInt);
			clients[myVector[j].myInt].sommetsVoisins.push_back(i);
			isCorrelated[myVector[j].myInt][i] = true;
		}
	}

	for (int i = 0; i < cli.nbDepots + nbClients; i++)
	{
		clients[i].computeVisitsDyn(cli.getNbDays(), oldNbDays);
		clients[i].computeJourSuiv(cli.getNbDays(), oldNbDays);
	}
}
...
void Params::getClient(int i, Client* myCli)
{
	...
	if (cli.type == Data::CARP || cli.type == Data::MDCARP) // CARP instances (also used when running MDCARP experiments, which only involve to define more depots)
	{
	...
	else if (cli.type == Data::NEARP) // NEARP instances (we don't really read the file here but at least initialize the structures)
	{
		myCli->ar_nodesExtr0 = -1;
		myCli->ar_nodesExtr1 = -1;
		myCli->demand = 0;
	...
	else if (cli.type == Data::PCARP) // PCARP instances
	...
	else if (cli.type == Data::NEARP_TP) // NEARP-TP instances (we don't really read the file here but at least initialize the structures)
	{
		myCli->demand = 0;
		if (i == 0)
		{
	...
	else if (cli.type == Data::MM_kWRPP) // Min-Max WRPP
	{
		myCli->serviceDuration = 0;
...
void Params::shuffleProches()
{
	...
	for (int i = cli.nbDepots; i < nbClients + cli.nbDepots; i++)
	{
		for (int a1 = 0; a1 < (int)clients[i].sommetsVoisins.size() - 1; a1++)
		{
			temp2 = a1 + rand() % ((int)clients[i].sommetsVoisins.size() - a1);
			temp = clients[i].sommetsVoisins[a1];
			clients[i].sommetsVoisins[a1] = clients[i].sommetsVoisins[temp2];
			clients[i].sommetsVoisins[temp2] = temp;
		}
	}

	for (int i = cli.nbDepots; i < nbClients + cli.nbDepots; i++)
	{
		for (int a1 = 0; a1 < (int)clients[i].sommetsVoisinsAvant.size() - 1; a1++)
		{
			temp2 = a1 + rand() % ((int)clients[i].sommetsVoisinsAvant.size() - a1);
			temp = clients[i].sommetsVoisinsAvant[a1];
			clients[i].sommetsVoisinsAvant[a1] = clients[i].sommetsVoisinsAvant[temp2];
			clients[i].sommetsVoisinsAvant[temp2] = temp;
		}
	}
}
...
void Params::ar_computeDistancesNodes()
{
	...
	for (int k = 1; k <= ar_NodesNonRequired + ar_NodesRequired; k++)
		for (int i = 1; i <= ar_NodesNonRequired + ar_NodesRequired; i++)
			for (int j = 1; j <= ar_NodesNonRequired + ar_NodesRequired; j++)
			{
				...
				if (cli.deadheadingArcs) ar_predNodes[i][j] = ar_predNodes[k][j];
			}
	...
	timeCost = new double*[nbClients + cli.nbDepots + 1];
	for (int i = 0; i < nbClients + cli.nbDepots; i++)
	{
		timeCost[i] = new double[nbClients + cli.nbDepots + 1];
		for (int j = 0; j < nbClients + cli.nbDepots; j++)
			timeCost[i][j] = min(min(ar_distanceNodes[clients[i].ar_nodesExtr0][clients[j].ar_nodesExtr0],
				ar_distanceNodes[clients[i].ar_nodesExtr0][clients[j].ar_nodesExtr1]),
				min(ar_distanceNodes[clients[i].ar_nodesExtr1][clients[j].ar_nodesExtr0],
					ar_distanceNodes[clients[i].ar_nodesExtr1][clients[j].ar_nodesExtr1]));
	}
}
...
void Params::ar_computeDistancesArcs()
{
	...
	timeCost = new double* [nbClients + cli.nbDepots];
	for (int i = 0; i < nbClients + cli.nbDepots; i++)
	{
		timeCost[i] = new double[nbClients + cli.nbDepots];
		for (int j = 0; j < nbClients + cli.nbDepots; j++)
		{
			myDistanceMin = 1.e20;
			for (int ii = 0; ii < (int)clients[i].ar_Modes.size(); ii++)
			{
				for (int jj = 0; jj < (int)clients[j].ar_Modes.size(); jj++)
				{
					myTemp = ar_distanceArcs[clients[i].ar_Modes[ii]->indexArc][clients[j].ar_Modes[jj]->indexArc];
...
void Params::ar_parseOtherLinesCARP()
{
	...
	for (int k = 0; k < ar_EdgesNonRequired; k++)
	{
		...
		if (cli.type != Data::MM_kWRPP) // not a "windy problem"
	...
	if (cli.type == Data::CARP)
	{
		fichier >> useless;
		fichier >> useless;
		fichier >> startNode; // in CARP instances, this information is included in the file

		clients[0].ar_nodesExtr0 = startNode;
		clients[0].ar_nodesExtr1 = startNode;
	}
	else if (cli.type == Data::MDCARP)
	{
		clients[0].ar_nodesExtr0 = 1;
		clients[0].ar_nodesExtr1 = 1;
		// Case with two depots (see instance specs in Kansou and Yassine 2009)
		if (cli.nbDepots == 2)
		{
			clients[1].ar_nodesExtr0 = ar_NodesNonRequired;
			clients[1].ar_nodesExtr1 = ar_NodesNonRequired;
		}
		else if (cli.nbDepots >= 3) // Case with three or more depots (see instance specs in Kansou and Yassine 2009)
			for (int d = 1; d < cli.nbDepots; d++)
			{
				clients[d].ar_nodesExtr0 = d * (int)(ar_NodesNonRequired / (cli.nbDepots - 1));
				clients[d].ar_nodesExtr1 = d * (int)(ar_NodesNonRequired / (cli.nbDepots - 1));
			}
	}
}
...
void Params::ar_parseOtherLinesNEARP()
{
	...
	for (int k = 0; k < ar_NodesRequired; k++)
	{
		...
		clients[iCour].ar_nodesExtr0 = idNode;
		clients[iCour].ar_nodesExtr1 = idNode;
		fichier >> clients[iCour].demand;
		totalDemand += clients[iCour].demand;
		fichier >> clients[iCour].ar_serviceCost01;
		if (cli.timeCapacitated)
			clients[iCour].ar_serviceCost10 = clients[iCour].ar_serviceCost01;
		else
		{
			clients[iCour].ar_serviceCost01 = 0; // to remove the base cost from the results.
			clients[iCour].ar_serviceCost10 = 0; // to remove the base cost from the results.
		}
		iCour++;
	}
	getline(fichier, contenu);
	...
	{
		fichier >> useless;
		fichier >> clients[iCour].ar_nodesExtr0;
		fichier >> clients[iCour].ar_nodesExtr1;

		// setting the distance between the nodes (here its symmetric because it's an edge)
		fichier >> myTravelCost;
		if (myTravelCost < ar_distanceNodes[clients[iCour].ar_nodesExtr0][clients[iCour].ar_nodesExtr1])
			ar_distanceNodes[clients[iCour].ar_nodesExtr0][clients[iCour].ar_nodesExtr1] = myTravelCost;
		if (myTravelCost < ar_distanceNodes[clients[iCour].ar_nodesExtr1][clients[iCour].ar_nodesExtr0])
			ar_distanceNodes[clients[iCour].ar_nodesExtr1][clients[iCour].ar_nodesExtr0] = myTravelCost;
		...
		{
			clients[iCour].ar_serviceCost01 = myTravelCost;
			clients[iCour].ar_serviceCost10 = myTravelCost;
		}
		iCour++;
	}
	getline(fichier, contenu);
	...
	for (int k = 0; k < ar_ArcsRequired; k++)
	{
		...
		if (!cli.timeCapacitated)
			clients[iCour].ar_serviceCost01 = myTravelCost;
		clients[iCour].ar_serviceCost10 = 1.e20; // This is an arc
		iCour++;
	}
	getline(fichier, contenu);
...
void Params::ar_parseOtherLinesNEARP_TP()
{
	...
	clients[0].ar_Modes.push_back(&ar_Arcs[0]);
	clients[0].ar_nbModes = 1;
	parsing_courNbArcs++;
	parsing_courService++;
	...
	for (int i = 0; i < this->ar_NodesNonRequired + this->ar_NodesRequired; i++)
	{
		...
		if (p_isReq)
		{
			clients[parsing_courService].ar_nodesExtr0 = p_indexI;
			clients[parsing_courService].demand = p_qty;
			totalDemand += clients[parsing_courService].demand;
			parsing_courService++;
		}
		fichier >> uselessDbl;
		fichier >> uselessDbl;
	...
	for (int i = 0; i < this->ar_EdgesNonRequired + this->ar_EdgesRequired; i++)
	{
		...
		if (p_isReq)
		{
			clients[parsing_courService].demand = p_qty;
			totalDemand += clients[parsing_courService].demand;
			clients[parsing_courService].ar_Modes.push_back(&ar_Arcs[parsing_courNbArcs - 2]);
			clients[parsing_courService].ar_Modes.push_back(&ar_Arcs[parsing_courNbArcs - 1]);
			clients[parsing_courService].ar_nbModes = 2;
			parsing_courService++;
		}
	}
	...
	for (int i = 0; i < this->ar_ArcsNonRequired + this->ar_ArcsRequired; i++)
	{
		...
		if (p_isReq)
		{
			clients[parsing_courService].demand = p_qty;
			totalDemand += clients[parsing_courService].demand;
			clients[parsing_courService].ar_Modes.push_back(&ar_Arcs[parsing_courNbArcs - 1]); // HERE is -1
			clients[parsing_courService].ar_nbModes = 1;
			parsing_courService++;
		}
	}
	...
	{
		clients[i].ar_nbModes = 0;
		for (int k = 0; k < (int)ar_Arcs.size(); k++)
			if (ar_Arcs[k].nodeEnd == clients[i].ar_nodesExtr0)
			{
				clients[i].ar_Modes.push_back(&ar_Arcs[k]);
				clients[i].ar_nbModes++;
			}
	}
	...
	for (int i = 0; i <= this->nbClients; i++)
		if (clients[i].ar_nbModes > ar_maxNbModes)
			ar_maxNbModes = clients[i].ar_nbModes;
}
...
//	cli.nbDays = cli.getNbDays() * cli.nbDepots;
//
...
//		{
//			ordreVehicules[k].push_back(ordreVehiculesAncien[(k - 1) % oldNbDays + 1][((k - 1) / oldNbDays) * nbVehiculesPerDep + d]);
//			dayCapacity[k] += ordreVehicules[k][d].vehicleCapacity;
//		}
//	}
//
//	// we update the patterns
//	for (int i = cli.nbDepots; i < cli.nbDepots + nbClients; i++)
//	{
//		nbPat = (int)clients[i].visitsOrigin.size();
//		clients[i].visits.clear();
//
//		for (int j = 0; j < cli.nbDepots; j++)
//		{
//			p.dep = j;
//			p.cost = 0;
//			for (int pat = 0; pat < nbPat; pat++)
//			{
//				p.pat = clients[i].visitsOrigin[pat].pat;
//				clients[i].visits.push_back(p);
//			}
//		}
//		clients[i].visitsOrigin = clients[i].visits;
//	}
//}
