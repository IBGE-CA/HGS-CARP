...
void Genetic::evolveHGA(int maxIterNonProd, int nbRec)
{
	...
	for (int i = 0; i < population->unfeasible.nbIndiv; i++)
		if (population->unfeasible.individuals[i]->coutSol.isBetterFeas(bestSolFeasibility)) bestSolFeasibility = population->unfeasible.individuals[i]->coutSol;
	...
	while (nbIterNonProd < maxIterNonProd && (clock() - debut <= ticks) && (!params->isSearchingFeasible || population->getIndividuBestValide() == NULL))
	{
		...
		// SOME TRACES
		if ((rejeton->estValide && place == 0) || (rejeton->coutSol.isBetterFeas(bestSolFeasibility) && population->feasible.nbIndiv == 0))
		{
