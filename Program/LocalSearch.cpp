...
int LocalSearch::searchBetterPattern(int client)
{
	...
	if (meilleurPattern.pat != pattern1.pat || meilleurPattern.dep != pattern1.dep || deplacementIntraJour)
	{
		...
		for (int k = 0; k < params->oldNbDays; k++)
		{
			...
			if (temp == 1)
			{
				Noeud* hereCli = &clients[params->oldNbDays - k + meilleurPattern.dep * params->oldNbDays][client];
...
void LocalSearch::computeCoutInsertion(Noeud* clients, int Pattern)
{
	...
	for (int r = 0; r < params->nombreVehicules[clients->jour]; r++)
	{
		myRoute = &routes[clients->jour][r];
	...
	if (clients->estPresent // its placed here
...
void LocalSearch::evalInsertClient(Route* R, Noeud* U, int Pattern)
{
	...
	U->seq1->load = params->clients[U->cour].demandPatDay[params->clients[U->cour].visits[Pattern].pat][U->jour];
	...
	for (int k = 1; k <= cli.getNbDays(); k++)
	{
		for (int i = 0; i < (int)ordreParcours[k].size(); i++)
		{
			...
			for (int a1 = 0; a1 < size; a1++)
			{
				...
				if (client2 >= cli.nbDepots && clients[k][client2].estPresent) clients[k][client].moves.push_back(client2);
			}
		}
	}
}
...
void LocalSearch::placeManquants()
{
	...
	for (int i = 0; i < (int)individu->toPlace.size(); i++)
	{
		...
		for (int kk = 0; kk < params->oldNbDays; kk++)
		{
			...
			{
				Noeud* hereCli = &clients[params->oldNbDays - kk + meilleurPattern.dep * params->oldNbDays][k];
				Noeud* thereCli = hereCli->placeInsertion[indexMeilleur];
				addNoeud(hereCli, thereCli);
			}
			calcul1 = calcul1 / 2;
...
LocalSearch::LocalSearch(Params* params, Individual* individu) : params(params), individu(individu)
{
	...
	clients = new Noeud*[cli.getNbDays() + 1];
	depots = new Noeud*[cli.getNbDays() + 1];
	depotsFin = new Noeud*[cli.getNbDays() + 1];
	routes = new Route*[cli.getNbDays() + 1];

	for (int kk = 1; kk <= cli.getNbDays(); kk++)
	{
		nbVehicles = params->nombreVehicules[kk];
		clients[kk] = new Noeud[params->nbClients + cli.nbDepots + 1];
		depots[kk] = new Noeud[nbVehicles];
		depotsFin[kk] = new Noeud[nbVehicles];
