...
bool CommandLineInterface::load(int argc, char* argv[])
{
	...
	if (type == Data::PCARP && nbVehicles <= 0)
	{
...
Data::Type CommandLineInterface::getType(const string& type)
{
	...
	else if (type == "PCARP") return Data::PCARP;
	else if (type == "MDCARP") return Data::MDCARP;
	else if (type == "NEARP_TP") return Data::NEARP_TP;
	else if (type == "MM_kWRPP") return Data::MM_kWRPP;
...
void CommandLineInterface::printHelp(const string& message)
{
	...
	cout << "  -type <value> Set problem type (CARP, NEARP, PCARP, MDCARP, NEARP_TP, MM_kWRPP - default = CARP)." << endl;
